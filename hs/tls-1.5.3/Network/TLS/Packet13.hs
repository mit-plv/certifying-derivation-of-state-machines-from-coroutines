...
decodeHandshakes13 bs = case decodeHandshakeRecord13 bs of
  ...
  GotPartial _cont            -> throwError $ Error_Protocol (show (B.length bs) ++ "decode_error", True, DecodeError)
...
decodeClientHello13 = do
    ...
    case mver of
        Nothing ->  fail "InvalidVersion"
        Just SSL2 -> fail "InvalidVersion"
        Just SSL3 -> fail "InvalidVersion"
        Just ver -> return $ ClientHello13 ver random session ciphers exts
