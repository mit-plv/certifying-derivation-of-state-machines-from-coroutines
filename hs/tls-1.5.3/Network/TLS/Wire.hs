...
instance Show (GetResult a) where
  show (GotError _) = "GotError"
  show (GotPartial _) = "GotPartial"
  show (GotSuccess _) = "GotSuccess"
  show (GotSuccessRemaining _ _) = "GotSuccessRemaining"

runGet :: String -> Get a -> ByteString -> GetResult a
runGet lbl f = toGetResult <$> G.runGetPartial (label lbl f)
  where toGetResult (G.Fail err _)    = if isInfixOf "InvalidVersion" err then GotError (Error_Protocol ("invalid version", True, ProtocolVersion)) else GotError (Error_Packet_Parsing err)
